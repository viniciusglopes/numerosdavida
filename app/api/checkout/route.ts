...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nome, destino } = body

    if (!MP_ACCESS_TOKEN) {
      return NextResponse.json({ error: 'Pagamento não configurado' }, { status: 500 })
    }

    const externalRef = `ndv_${Date.now()}_${destino}`

    const preference = {
      items: [{
        title: 'Mapa Numerológico Completo - Números da Vida',
        description: `Mapa personalizado para ${(nome || '').split(' ')[0]} - Destino ${destino}`,
        quantity: 1,
        currency_id: 'BRL',
        unit_price: 9.90,
      }],
